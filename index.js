...
function searchMeal() {
    ...
    if (!userInput) {
        resultContainer.innerHTML = `<h3 style="color: white; animation: blink 3.5s linear infinite;">Please Enter Meal</h3>
            <style>
			    @keyframes blink {
			        0% {
			            opacity: 1;
    ...
    fetch(apiUrl + userInput).then((response) => response.json()).then((data) => {
        ...
        .catch(() => {
            ...
            resultContainer.innerHTML = `<h3 style="color: white; animation: blink 3.5s linear infinite;">No Recipe Found, Please Try Again!</h3>
            <style>
			    @keyframes blink {
			        0% {
			            opacity: 1;
